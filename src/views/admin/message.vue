                ...
                <div class="ion-page" id="main-content">
                    ...
                    <ion-content>
                        ...
                        <div class="leave-container">
                            <div v-if="totaladminListId && totaladminListId.rows">
                                <ion-list>
                                    <ion-item-sliding v-for="message in totaladminListId.rows" :key="message.id">
                                        <ion-item>
                                            ...
                                            <ion-label>
                                                <h2 v-if="message.createdBy && message.createdBy.id === parseInt(userId1)">
                                                    Sent to: {{ message.user?.role?.roletype }} - {{ message.user?.username
                                                    }}
                                                </h2>
                                                <h2 v-else-if="message.admin && message.admin.id === parseInt(userId1)">
                                    ...
                                    <ion-row>
                                        <ion-col>
                                            <ion-select v-model="userId" placeholder="Select user">
                                                <ion-select-option v-for="user in filteredUsers" :key="user.id"
                                                    :value="user.id">
...
export default defineComponent({
    ...
    data() {
        return {
            ...
            showDelete: false,
            debug: true,
            allowedRoles: ['counseller staff', 'documentation staff', 'application staff', 'Add lead']
        };
    },
    computed: {
        ...mapState('message', ['totaladminListId',]),
        ...mapState('user', ['totalstaffList',]),
        filteredUsers() {
            if (!this.totalstaffList) {
                console.warn('totalstaffList is undefined or null');
                return [];
            }

            return this.totalstaffList.filter(user => {
                // Add debugging logs
                if (this.debug) {
                    console.log('Filtering user:', user.username, 'Role:', user.role);
                }

                // Check if user has role and it matches allowed roles
                return user.role &&
                    user.role.roletype &&
                    this.allowedRoles.map(role => role.toLowerCase()).includes(user.role.roletype.toLowerCase());

            });
        }
    },
    methods: {
        ...mapActions('message', ['fetchtotaladminlistId', 'ADD_MESSAGE', 'update_message', 'delete_message']),
        ...mapActions('user', ['fetchtotalstafflist']),
        ...
        async openUpdateModal(leaveId) {
            ...
            try {
                const leadData = await this.totaladminListId.rows.find((message) => message.id === leaveId);
        ...
        async fetchleadData() {
            ...
            try {
                await this.fetchtotalstafflist();
                await this.fetchtotaladminlistId(userId);
