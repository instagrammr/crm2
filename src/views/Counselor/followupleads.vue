      ...
      <ion-split-pane content-id="main-content">
        ...
        <div class="ion-page" id="main-content">
          <ion-header>
            <ion-toolbar>
              ...
              <ion-buttons slot="end">
                <ion-searchbar v-model="searchQuery" placeholder="Search by name, country or mobile"
                  class="custom-searchbar" @ionChange="handleSearch"></ion-searchbar>
              </ion-buttons>
          ...
          <ion-content>
            ...
            <!-- Student Cards -->
            <div v-if="filteredFollowups.length" class="cards-container">
              <ion-card class="student-info-card" v-for="lead in filteredFollowups" :key="lead.id"
                :class="{ 'expanded': expandedId === lead.id }" @click="toggleExpand(lead.id)">
                <ion-card-content>
                    ...
                    <div class="student-details">
                      ...
                      <div class="info-row">
                        ...
                        <div class="info-item">
                          ...
                          <span v-if="lead.followup_date">followup date: {{ new
                            Date(lead.followup_date).toISOString().split('T')[0] }}</span>
...
import {
  IonApp, IonContent, IonHeader, IonToolbar, IonTitle, IonModal,
  IonButtons, IonMenuButton, IonButton, IonIcon, IonList, IonLabel,
  IonSplitPane, IonGrid, IonRow, IonCol, IonSearchbar, IonBadge,
  IonAvatar, IonCard, IonCardContent, IonPage, IonSelectOption, IonSelect,
  IonTextarea, IonInput, IonItem
} from '@ionic/vue';
import {
  peopleOutline, personOutline, timeOutline, locationOutline, closeOutline,
  ellipsisHorizontal, star, starSharp, mailOutline, schoolOutline,
  calendarOutline, chatbubbleOutline, checkmarkOutline, searchOutline, eyeOutline,
  checkmarkCircleOutline, alertCircleOutline, closeCircleOutline, notificationsOutline
} from 'ionicons/icons';
...
export default defineComponent({
  ...
  components: {
    ...
    IonSelectOption, IonSelect, IonBadge, IonList, IonLabel,
    IonTextarea, IonInput, IonItem
  },
  data() {
    return {
      ...
      locationOutline,
      ellipsisHorizontal, star, starSharp, mailOutline, schoolOutline,
      calendarOutline, chatbubbleOutline, checkmarkOutline, peopleOutline,
      checkmarkCircleOutline, alertCircleOutline, timeOutline, personOutline,
  ...
  computed: {
    ...mapState('counsiller', ['totalfollowupListId', 'totalFollowupCount', 'totalCounsillerList', 'totalremarkListId', 'totaltodayLeadCount', 'totaluploadListId', 'totalLeadCount']),
    notificationCount() {
    ...
    filteredFollowups() {
      if (!this.totalfollowupListId?.rows) return [];

      return this.totalfollowupListId.rows.filter(lead => {
    ...
    activeCount() {
      return this.totalfollowupListId?.rows?.filter(lead => lead.status === 'active').length || 0;
    },

    pendingCount() {
      return this.totalfollowupListId?.rows?.filter(lead => lead.status === 'pending').length || 0;
    },

    overdueCount() {
      return this.totalfollowupListId?.rows?.filter(lead => lead.status === 'overdue').length || 0;
    }
  },

  methods: {
    ...mapActions('counsiller', ['fetchtotalfollowuplistId', 'fetchtotalfollowupCount', 'fetchtotalCounsillerlist', 'update_lead', 'Add_remark',
      'fetchtotalremarkListId', 'update_remark', 'delete_remark', 'fetchtotaltodayLeadCount', 'FETCH_DOCUMENTS', 'fetchtotalleadCount']),
    ...
    showOptions(lead) {
      ...
      console.log('Show options for:', lead.id);
    },
    scheduleFollowup(lead) {
      // Implement schedule follow-up logic
      console.log('Schedule follow-up for:', lead.id);
    },
    updateStatus(lead) {
      // Implement status update logic
      console.log('Update status for:', lead.id);
    },
    async fetchleadData() {
